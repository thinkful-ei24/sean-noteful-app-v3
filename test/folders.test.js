      ...
      return Promise.all([
        ...
        .then(([data, res]) => {
          ...
          res.body.forEach(function (item, i) {
            ...
            expect(item.id).to.equal(data[i].id.toString());
    ...
    it('should return correct folder', function () {
      ...
      return Folder.findOne()
        ...
        .then((res) => {
          ...
          expect(res.body.id).to.equal(data.id.toString());
  ...
  describe('PUT /api/folders/:id', function () {
    ...
    it('should update the folder', function () {
      ...
      return Folder.findOne({userId: user.id})
        .then(_data => {
          data = _data;
          return chai.request(app).put(`/api/folders/${data.id}`).set('Authorization', `Bearer ${token}`)
            .send(updateItem);
        })
        .then(function (res) {
          expect(res).to.have.status(200);
    ...
    it('should respond with a 400 for an invalid id', function () {
      ...
      return chai.request(app).put('/api/folders/NOT-A-VALID-ID')
        .set('Authorization', `Bearer ${token}`)
        .send(updateItem)
        .then(res => {
          expect(res).to.have.status(400);
